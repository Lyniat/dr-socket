...
extern "C" {
    ...
    void drb_register_c_extensions_with_api(mrb_state *state, struct drb_api_t *api) {
        ...
        drb_api->mrb_define_module_function(state, module, "socket_client_send_to_server",{[](mrb_state *mrb, mrb_value self){
            mrb_value hash, data;
            drb_api->mrb_get_args(mrb,"H", &hash);
            ...
            int size = serialize_data_to_buffer(buffer, 1024 * 1024, 0, serialized_data);
