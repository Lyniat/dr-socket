...
serialized_data_t serialize_data(mrb_state *mrb, mrb_value data){
    ...
    if(type == MRB_TT_STRING){
        ...
        serialized_data_t serialized_string = {.type = MRB_TT_STRING, .data = (void*)string_dup, .size = (int)(str_len +1), .amount = 0};
    ...
    if(type == MRB_TT_SYMBOL){
        const char* string = drb_api->mrb_sym_name(mrb, drb_api->mrb_obj_to_sym(mrb, data));
        //printf("%s\n", string);
        size_t str_len = strlen(string);
        const char* string_dup = strdup(string);
        serialized_data_t serialized_string = {.type = MRB_TT_SYMBOL, .data = (void*)string_dup, .size = (int)(str_len +1), .amount = 0};
        return serialized_string;
    }

    if(type == MRB_TT_HASH){
        mrb_value keys = drb_api->mrb_hash_keys(mrb, data);
        mrb_value key = drb_api->mrb_ary_shift(mrb, drb_api->mrb_ensure_array_type(mrb, keys));
        std::vector<serialized_hash_t> data_vector;
        int data_size = 0;
        int hash_size = 0;
        while (cext_is_symbol(mrb, key)){
            const char* s_key = strdup(drb_api->mrb_sym_name(mrb, drb_api->mrb_obj_to_sym(mrb, key)));
            mrb_value content = drb_api->mrb_hash_get(mrb, data, key);
    ...
    if(type == MRB_TT_ARRAY){
        ...
        mrb_value object = drb_api->mrb_ary_shift(mrb, drb_api->mrb_ensure_array_type(mrb, data));
        int data_size = 0;
        int array_size = 0;
...
mrb_value deserialize_data(mrb_state *mrb,const char* buffer, int size, int* position){
    ...
    if(type == MRB_TT_SYMBOL){
        ...
        int data_size = buffer[*position];
        *position += sizeof(int);
        mrb_value data = drb_api->mrb_symbol_value(cext_sym(mrb, buffer + *position));
        *position += data_size;
        return data;
    }
    if(type == MRB_TT_INTEGER){
        ...
        *position += sizeof(mrb_int);
        return drb_api->mrb_int_value(mrb, *integer);
    }
    if(type == MRB_TT_FLOAT){
        ++*position;
        mrb_float* d = (mrb_float*)(buffer + *position);
        *position += sizeof(mrb_float);
        return drb_api->mrb_float_value(mrb, *d);
    }
    if(type == MRB_TT_HASH){
        ++*position;
        int data_size = buffer[*position];
